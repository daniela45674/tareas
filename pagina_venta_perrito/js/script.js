...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (inputBuscar) {
        // Si ya estamos en productos.html, recuperamos el texto si viene por URL
        const urlParams = new URLSearchParams(window.location.search);
        const terminoPrevio = urlParams.get("busqueda");
        if (terminoPrevio) {
            inputBuscar.value = terminoPrevio;
        }
        ...
        inputBuscar.addEventListener("keydown", (event) => {
            if (event.key === "Enter") {
                ...
                if (query !== "") {
                    // Redirige a productos.html enviando el parámetro de búsqueda
